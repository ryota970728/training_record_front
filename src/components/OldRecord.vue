  ...
  <div class="record-search">
    ...
    <div class="search-area-wrapper">
      ...
      <div class="part-container">
        <label v-for="part in partList" :key="part.part_id" class="part-radio">
    ...
    <div class="search-result-wrapper">
      ...
      <div v-for="record in filterRecordList" :key="record.record_id" class="search-result-container">
        ...
        <div class="set-detail">
          <div v-for="(sets, index) in record.old_set_detail" :key="index">
...
export default {
  ...
  async mounted() {
    this.partList = await apiService.fetchOldPart();
    this.menuList = await apiService.fetchOldMenu();
    this.recordList = await apiService.fetchOldRecords();
  },
  computed: {
  ...
  methods: {
    searchRecordData() {
      ...
      this.filterRecordList = this.recordList.filter(record => {
        // 選択された種目でフィルタリング
        return this.selectedMenu == record.old_menu_master.menu_name;
      })
      .sort((a, b) => {
        // 日付でソート（降順）
